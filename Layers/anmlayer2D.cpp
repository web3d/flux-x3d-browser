	...
	m_transparency( ANMLAYER2D_DEFAULT_TRANSPARENCY )
{
	...
	m_OrthoParams[0] = 
	m_OrthoParams[2] = 
	m_OrthoParams[4] = 1.0f;
	m_OrthoParams[1] = 
	m_OrthoParams[3] = 
	m_OrthoParams[5] = 0.0f;
}
...
void CAnmLayer2D::Traverse(class CAnmUpdateVisitor *pVisitor)
{
	CAnmLayer::Traverse(pVisitor);


}

void CAnmLayer2D::Update()
{
	UpdateLayerData();
	CAnmLayer::Update();
}
...
void CAnmLayer2D::RecalculateCameraParams(CAnmDevice *pDevice)
{
	...
	// krv
	// Do we need to call UpdateLayerData again to be sure its up to date????
	//
	UpdateLayerData();

	// Set Ortho Params.
	//
	m_camera->SetOrthoParams( 
								m_OrthoParams[0], 
								m_OrthoParams[1], 
								m_OrthoParams[2], 
								m_OrthoParams[3], 
...
//	assert(pWorld);
	...
	float scaleToPixelFactor = fVPOrthoWidth  / fOverlayPixelWidth;

	// Scale the X points
	//
	float factor = 1.0f;
	
	factor = 1.0f / scaleToPixelFactor;
	...
	// Pick a nice scale for Z.
	//
	scaleZ = ( scaleX + scaleY ) / 2.0;

	// Modify the ortho params so the clipping planes know about our new scale.
	z0 *= scaleZ;
	z1 *= scaleZ;
...
//	m_camera->SetOrthoParams( x0 - 0.5f, x1 - 0.5f, y0 + 0.5f, y1 + 0.5f, z0, z1 );
	...
	m_OrthoParams[0] = x0 - 0.5f;
	m_OrthoParams[1] = x1 - 0.5f;
	m_OrthoParams[2] = y0 + 0.5f;
	m_OrthoParams[3] = y1 + 0.5f;
	m_OrthoParams[4] = z0;
	m_OrthoParams[5] = z1;
...
// member table
BEGIN_DEFINE_NODE_MEMBERS(CAnmLayer2D)
