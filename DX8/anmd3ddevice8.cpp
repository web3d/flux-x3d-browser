...
CAnmD3DDevice8::CAnmD3DDevice8(HWND hWnd, cApplication *pApp)
...
void CAnmD3DDevice8::DestroyAll()
{
    if( m_pDevice )
	{
...
CAnmD3DDevice8::~CAnmD3DDevice8()
{
	DestroyAll();
}


void CAnmD3DDevice8::Clear( 
	bool bClearFrame, 
	bool bClearZ, 
	DWORD frameColor, 
	float zValue )
{
	...
	if( FAILED( hr ))
	{
		DP0("[CAnmD3DDevice8::Clear]: Clear failed\n");
	}
}

static gAnmFlipCount = 0;
static gAnmFlipFailureCount = 0;
void CAnmD3DDevice8::Flip( bool bNoVSync )
{
	HRESULT hr;
...
void CAnmD3DDevice8::GetWorldMatrix( matrix4* pMat )
{
...
void CAnmD3DDevice8::SetWorldMatrix( const matrix4& mat )
{
	// Pass the call to the Direct3D Device
...
void CAnmD3DDevice8::GetViewMatrix( matrix4* pMat )
{
	// Pass the call to the Direct3D Device
	m_pDevice->GetTransform( 
...
void CAnmD3DDevice8::SetViewMatrix( const matrix4& mat )
{
	// Pass the call to the Direct3D Device
	m_pDevice->SetTransform( 
		D3DTRANSFORMSTATE_VIEW, 
...
void CAnmD3DDevice8::GetProjectionMatrix( matrix4* pMat )
{
	// Pass the call to the Direct3D Device
	m_pDevice->GetTransform( 
...
void CAnmD3DDevice8::SetProjectionMatrix( const matrix4& mat )
{
	// Pass the call to the Direct3D Device
	m_pDevice->SetTransform( 
		D3DTRANSFORMSTATE_PROJECTION, 
...
void CAnmD3DDevice8::SetOrthoMatrix( float x0, float x1, float y0, float y1, float z0, float z1 )
{
	D3DXMATRIX matView;
...
void CAnmD3DDevice8::SetViewportParams(int x, int y, int width, int height, float minz, float maxz)
{
    D3DVIEWPORT8 vp = { x, y, width, height, minz, maxz };
...
void CAnmD3DDevice8::BeginScene()
{
	// Pass the call to the Direct3D Device
	m_pDevice->BeginScene();
}

void CAnmD3DDevice8::EndScene()
{
	// Pass the call to the Direct3D Device
	m_pDevice->EndScene();
}

void CAnmD3DDevice8::InitDevice()
{
	HRESULT hr;
...
DWORD
CAnmD3DDevice8::GetCreateDeviceFlags()
{
	return m_createDeviceFlags;
}

void
CAnmD3DDevice8::GetViewMatrix(D3DXMATRIX* viewMatrix)
{
	m_pDevice->GetTransform(D3DTRANSFORMSTATE_VIEW, viewMatrix);
}

void
CAnmD3DDevice8::GetProjectionMatrix(D3DXMATRIX* projMatrix)
{
	m_pDevice->GetTransform(D3DTRANSFORMSTATE_PROJECTION, projMatrix);
}

void
CAnmD3DDevice8::GetWorldMatrix(D3DXMATRIX* worldMatrix)
{
	m_pDevice->GetTransform(D3DTRANSFORMSTATE_WORLD, worldMatrix);
}

D3DMULTISAMPLE_TYPE CAnmD3DDevice8::GetMultiSampleType() 
{
	...
	mType = D3DMULTISAMPLE_4_SAMPLES;
	hr = m_pD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, m_devType,
													m_d3ddm.Format, TRUE, mType);
	if(SUCCEEDED(hr)) {
		return mType;
	}
	...
	mType = D3DMULTISAMPLE_2_SAMPLES;
	hr = m_pD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, m_devType,
													m_d3ddm.Format, TRUE, mType);
	if(SUCCEEDED(hr)) {
		return mType;
...
//-----------------------------------------------------------------------------
D3DFORMAT CAnmD3DDevice8::GetDepthStencilFormat()
{
...
//-----------------------------------------------------------------------------
HRESULT CAnmD3DDevice8::ConfirmDevice( D3DCAPS8* pCaps, DWORD dwBehavior,
                                          D3DFORMAT Format )
{
...
//       with the render target format and meets the needs of the app.
//-----------------------------------------------------------------------------
BOOL CAnmD3DDevice8::FindDepthStencilFormat( UINT iAdapter, D3DDEVTYPE DeviceType,
...
void CAnmD3DDevice8::DrawTextString( 
	int x, int y, 
...
//    HRESULT hr;
	...
	if(SUCCEEDED(m_pDevice->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer))) {
...
void CAnmD3DDevice8::InitD3D()
{
	...
	if( m_pD3D == NULL )
	{
		// ANMMSG_D3DCREATE8FAILED : "Direct3DCreate8 failed"
        throw CAnmError(ANMMSG_D3DCREATE8FAILED);
	}
...
//	throw CAnmError("Forced throw in CreateDevice");
	...
	m_createDeviceFlags = D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_MULTITHREADED  | D3DCREATE_FPU_PRESERVE;

	hr = ConfirmDevice(&pCaps, m_createDeviceFlags, m_d3dpp.BackBufferFormat);
	if(FAILED(hr)) {
		m_createDeviceFlags = D3DCREATE_MIXED_VERTEXPROCESSING | D3DCREATE_MULTITHREADED | D3DCREATE_FPU_PRESERVE ;
		hr = ConfirmDevice(&pCaps, m_createDeviceFlags, m_d3dpp.BackBufferFormat);
		if(FAILED(hr)) {
			m_createDeviceFlags = D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_MULTITHREADED | D3DCREATE_FPU_PRESERVE;
			hr = ConfirmDevice(&pCaps, m_createDeviceFlags, m_d3dpp.BackBufferFormat);
			if(FAILED(hr)) {
	...
	// throw CAnmError("testing error recovery");
	...
	if(FAILED(hr)) {
		...
		{
			// last ditch; try turning off antialiasing
...
void CAnmD3DDevice8::HandleSize(int newwidth, int newheight)
{
...
void CAnmD3DDevice8::HandleMove()
{
//    D3DVIEWPORT8 vp;

    // HRESULT hr = m_pDevice->GetViewport( &vp );

}

void CAnmD3DDevice8::HandleDeviceLost()
{
...
void CAnmD3DDevice8::Reset()
{
	...
	if (FAILED(hr))
	{
		...
		else if (m_d3dpp.MultiSampleType != D3DMULTISAMPLE_NONE)
		{
			// last ditch; try turning off antialiasing
			m_d3dpp.MultiSampleType = D3DMULTISAMPLE_NONE;
		...
		if (!ok)
		{
			//DX8 documentation says that the application should pop-up message
...
bool CAnmD3DDevice8::IsDeviceReady()
{
	return m_pDevice != NULL;
}


void CAnmD3DDevice8::SetAntialiasing(bool antialias)
{
	HRESULT hr = S_FALSE;
...
void CAnmD3DDevice8::SetWireframe(bool wireframe)
{
	HRESULT hr = S_FALSE;
...
// Materials help
void CAnmD3DDevice8::SetMaterial(CAnmGraphics::Material *pMaterial)
{
	CAnmD3DMaterial8 *pMaterial8 = (CAnmD3DMaterial8 *) pMaterial;

	D3DMATERIAL8 *pD3DMaterial8 = pMaterial8->GetD3DMaterial8();

	HRESULT hr = m_pDevice->SetMaterial(pD3DMaterial8);
}


// Textures
void CAnmD3DDevice8::SetTexture(int stage, CAnmGraphics::Texture *pTexture)
{
	CAnmD3DTextureSurface8 *pTextureSurface = (CAnmD3DTextureSurface8 *) pTexture;
...
void CAnmD3DDevice8::SetTextureOption(int stage, eAnmTextureOptions option, int value)
{
	DWORD d3dmode = 0L;
	...
	switch (option)
	{
		case eAnmTextureAddressU :
			m_pDevice->SetTextureStageState(stage, D3DTSS_ADDRESSU, AnmNativeTextureAddressMode((eAnmTextureAddressMode)value));
			break;

		case eAnmTextureAddressV :
			m_pDevice->SetTextureStageState(stage, D3DTSS_ADDRESSV, AnmNativeTextureAddressMode((eAnmTextureAddressMode)value));
			break;

		case eAnmTextureFilter :
			if (value)
			{
				m_pDevice->SetTextureStageState(stage, D3DTSS_MAGFILTER,   D3DTEXF_LINEAR);
				m_pDevice->SetTextureStageState(stage, D3DTSS_MINFILTER,   D3DTEXF_LINEAR);
				m_pDevice->SetTextureStageState(stage, D3DTSS_MIPFILTER,   D3DTEXF_LINEAR);
			}
			else
			{
				m_pDevice->SetTextureStageState(stage, D3DTSS_MAGFILTER,   D3DTEXF_POINT);
				m_pDevice->SetTextureStageState(stage, D3DTSS_MINFILTER,   D3DTEXF_POINT);
				m_pDevice->SetTextureStageState(stage, D3DTSS_MIPFILTER,   D3DTEXF_NONE);
			}
			break;
...
void CAnmD3DDevice8::SetTextureTransform(int stage, matrix4 *pTT)
{
	assert(pTT);
...
void CAnmD3DDevice8::GetDefaultTextureTransform(matrix4 *pDefaultTT)
{
	assert(pDefaultTT);
...
// Lighting help
void CAnmD3DDevice8::SetLight(int index, CAnmGraphics::Light *pLight)
{
	CAnmD3DLight8 *pLight8 = (CAnmD3DLight8 *) pLight;

	D3DLIGHT8 *pD3DLight8= pLight8->GetD3DLight8();

	HRESULT hr = m_pDevice->SetLight(index, pD3DLight8);
}

void CAnmD3DDevice8::LightEnable(int index, bool on)
{
	HRESULT hr = m_pDevice->LightEnable(index, on);
}

void CAnmD3DDevice8::SetLighting(bool on)
{
	HRESULT hr = m_pDevice->SetRenderState(D3DRENDERSTATE_LIGHTING, on ? TRUE : FALSE);
}

void CAnmD3DDevice8::EnableFog(bool on)
{
	HRESULT hr;
...
void CAnmD3DDevice8::SetFog(CAnmGraphics::Fog *pFog)
{
	assert(pFog);

	CAnmD3DFog8 *pFog8 = (CAnmD3DFog8 *) pFog;

	HRESULT hr;

	hr = m_pDevice->SetRenderState(D3DRENDERSTATE_FOGCOLOR, pFog8->fogcolor);
	// hr = m_pDevice->SetRenderState(D3DRENDERSTATE_FOGTABLEMODE, pFog7->fogmode);
	hr = m_pDevice->SetRenderState(D3DRENDERSTATE_FOGVERTEXMODE, pFog8->fogmode);
	hr = m_pDevice->SetRenderState(D3DRENDERSTATE_FOGSTART, pFog8->fogstart);
	hr = m_pDevice->SetRenderState(D3DRENDERSTATE_FOGEND, pFog8->fogend);
	hr = m_pDevice->SetRenderState(D3DRENDERSTATE_FOGDENSITY, pFog8->fogdensity);
}

// Render states
void CAnmD3DDevice8::SetZBuffering(bool enabled)
{
	HRESULT hr;
...
void CAnmD3DDevice8::SetRenderOptions(eAnmRenderOptions options)
{
	HRESULT hr;
...
// Alpha blending
void CAnmD3DDevice8::SetAlphaBlending(bool alphablending)
{
	if (alphablending)
	{
...
void CAnmD3DDevice8::SetVertexShader(CAnmGraphics::Shader *pVertexShader)
{
	m_vertexShader = pVertexShader;

	// Not implemented; just save the pointer
}

void CAnmD3DDevice8::SetFragmentShader(CAnmGraphics::Shader *pFragmentShader)
{
